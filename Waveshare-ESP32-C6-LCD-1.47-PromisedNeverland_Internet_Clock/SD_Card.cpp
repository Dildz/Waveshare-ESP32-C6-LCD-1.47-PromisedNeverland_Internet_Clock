...
bool createJSONFile(const char* path, const JsonDocument& doc) {
  if(SD.exists(path)) {
...
bool readJSONFile(const char* path, JsonDocument& doc) {
  ...
  printf("Successfully read JSON file: %s\n", path);
  return true;
}
...
bool writeJSONFile(const char* path, const JsonDocument& doc) {
  ...
  serializeJson(doc, file);
  file.close();
  printf("Successfully wrote to JSON file: %s\n", path);
  return true;
}