...
void NeoPixel_Init(uint8_t initialBrightness) {
  Set_Brightness(initialBrightness);
}

void Set_Brightness(uint8_t brightness) {
  currentBrightness = brightness;
  // Apply brightness to current color immediately
  Set_Color(currentRed, currentGreen, currentBlue);
}
...
void Set_Color(uint8_t Red, uint8_t Green, uint8_t Blue) {
  ...
  uint8_t scaledRed = (Red * currentBrightness) / 255;
  uint8_t scaledGreen = (Green * currentBrightness) / 255;
  uint8_t scaledBlue = (Blue * currentBrightness) / 255;
  
  rgbLedWrite(PIN_NEOPIXEL, scaledRed, scaledGreen, scaledBlue);
}

void NeoPixel_Loop(uint16_t Waiting) { 
  Time++;
  if(Time == Waiting) {
    Time = 0;
    Number++;
